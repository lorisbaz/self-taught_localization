function ILSVRC2012_200rnd_VAL_detection_AP_diff_OBFSgt_testing_OBFSgt()
...
avg_prec = mean(sort(OBFSgt_testing.average_precision(idxs)-OBFSgt.average_precision(idxs)))*100;
% avg_prec = mean(sort(OBFSgt_testing.average_precision-OBFSgt.average_precision))*100;
legend_string = sprintf('%s\n-minus-\n%s\n(AVG=%.4f)', OBFSgt_testing.legend, OBFSgt.legend, avg_prec);
...
this_legend = plot_detection_graph(sort(OBFSgt_testing.average_precision(idxs)-OBFSgt.average_precision(idxs)), [1:sum(idxs)], 'o', legend_string, OBFSgt_testing.color);
