function ILSVRC2012_200rnd_VAL_detection_AP_best_worst_categories_with_STL_testing()

config;

% ----------- Table top-C, bottom-C classes ----------- %
[~, GTdet.idx_sort] = sort(GTdet.average_precision, 'descend');
[~, OBFSgt_testing.idx_sort] = sort(OBFSgt_testing.average_precision, 'descend');
...
OBFSgt_testing.average_precision = OBFSgt_testing.average_precision .* 100;
...
fprintf('\\begin{tabular}{|r|r|r|r|}\\hline \n');
fprintf(' %s & %s & %s & %s & %s \\\\ \\hline \n', GTdet.legend, OBFSgt_testing.legend, OBFSgt.legend, SS.legend, BING.legend);
% top
for k=1:topC
    fprintf('\\best{%s = %1.2f} & \\best{%s = %1.2f} & \\best{%s = %1.2f} & \\best{%s = %1.2f} \\\\ ', ...
        ...
        crop_class_name(classs_description_200{OBFSgt_testing.idx_sort(k)}, N_words), ...
        OBFSgt_testing.average_precision(OBFSgt_testing.idx_sort(k))*factor,...
...
fprintf('\\hline \n')
fprintf('mAP = %1.2f & mAP = %1.2f & mAP = %1.2f & mAP = %1.2f & mAP = %1.2f \\\\ ', ...
        mean(GTdet.average_precision)*factor,...
        mean(OBFSgt_testing.average_precision)*factor,...
