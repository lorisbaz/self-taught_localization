...
def pipeline(inputdb, outputdb, params):
    ...
    db_input.close()
    # extract the ss bboxes, 
    # and append them to the 'SELECTIVESEARCH' pred_objects
    logging.info('Extracting Selective Search bounding boxes')
    ss_bboxes = selective_search(images, params.ss_version)
    logging.info('Done.');
    ss_label = 'none'
    for idx in range(len(anno_imgs)):
        anno_img = anno_imgs[idx]
        anno_obj = AnnotatedObject(label=ss_label)
        anno_obj.bboxes.extend( ss_bboxes[idx] )
        anno_img.pred_objects['SELECTIVESEARCH'] = {}
        anno_img.pred_objects['SELECTIVESEARCH'][ss_label] = anno_obj
